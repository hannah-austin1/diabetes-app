...
async function DiabetesContent() {
  ...
  const windowStart = yesterdayMidnight.getTime();
  ...
  // Build unified timeline for the roller coaster
  const timeline: TimelineEvent[] = [
    // Nightscout treatments (carbs + boluses)
    ...treatments
      .filter((t) => t.mills >= windowStart && t.mills <= windowEnd)
      .map((t) => {
        if (t.carbs != null && t.carbs > 0) {
          return { ts: t.mills, kind: "carbs" as const, label: `${t.carbs}g carbs`, emoji: "🍞" };
        }
        return { ts: t.mills, kind: "insulin" as const, label: `${t.insulin}u insulin`, emoji: "💉" };
      }),
    // Finch completed goals
    ...finchEvents
      .filter((e) => e.kind === "goal")
      .map((e) => ({ ts: e.ts, kind: "goal" as const, label: e.label, emoji: e.emoji ?? "✅" })),
  ].sort((a, b) => a.ts - b.ts);
              ...
              if (h.count === 0) {
                return (
                  <div key={h.hour} className="flex-1 h-full flex flex-col items-center justify-end">
                    ...
                    {h.hour % 6 === 0 && (
                      <span className="text-muted-foreground font-mono mt-1" style={{ fontSize: 9 }}>{fmt(h.hour)}</span>
                    )}
                  </div>
                );
              }
